...
def block_feature(grad_mag, grad_angle):
    ...
    for i in range(h//8):
        for j in range(w//8):
            block_histogram[i, j] = block_extract(grad_mag[8*i:8*i + 8, 8*j:8*j + 8], grad_angle[8*i:8*i + 8, 8*j:8*j + 8])

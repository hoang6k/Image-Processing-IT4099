...
def cell_extract(grad_mag, grad_angle):
    feature = np.zeros(9)
...
def block_feature(grad_mag, grad_angle):
    h = grad_mag.shape[0]
    w = grad_mag.shape[1]
    cell_histogram = np.zeros((h // 8, w // 8, 9))
    feature_block = np.zeros((h // 8 - 1, w // 8 - 1, 36))
    for i in range(h // 8):
        for j in range(w // 8):
            cell_histogram[i, j] = cell_extract(grad_mag[8 * i:8 * (i + 1), 8 * j:8 * (j + 1)],
                                                grad_angle[8 * i:8 * (i + 1), 8 * j:8 * (j + 1)])
    for i in range(h // 8 - 1):
        for j in range(w // 8 - 1):
            feature = np.concatenate(cell_histogram[i:i + 2, j:j + 2], axis=None)
            try:
                feature_block[i, j] = feature / np.linalg.norm(feature)
            except ZeroDivisionError:
                feature_block[i, j] = feature
...
if __name__ == "__main__":
    ...
    # np.save('feature_pos_train.npy', data_pos)
    # data_pos = np.load('feature_pos_train.npy')

    # data_neg = feature_extraction(train_neg)
    # np.save('feature_neg_train.npy', data_neg)
